...
const SCHEMA = `Each question is a JSON object:
{
  ...
  "choices": string[],                  // choice only: the correct answer + 6 plausible wrong ones (7 total). The app shows 4 at a time, sampled at random, so every wrong option must be genuinely wrong and believable on its own. LENGTH RULE: all 7 options must be about the same length (within a few characters) — never let the correct one be the longest or the most detailed. If the answer needs a qualifier ("because it traps heat"), give the wrong ones the same kind of qualifier. Also match their grammar, specificity and tone so no option stands out.
