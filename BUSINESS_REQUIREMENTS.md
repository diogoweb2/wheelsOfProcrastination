...
- **Real market moves**: `npm run bank:market` (monthly cron, Yahoo Finance chart API — no API key) fetches ~30 days of real XGRO/QQQ daily % returns (adjusted close) into Firestore `app/marketData`; the sim replays them for the next ~30 days (looping if the next fetch is late). On failure it flags `status:"failed"` (keeping the last good series), retries daily, and the app shows a **red banner on Diogo's dashboard + Banker's desk**. When no series is available the sim falls back to the admin's monthly-rate estimates.
...
Code: `src/logic/gym.ts` (the offline brain), `src/logic/gymBlock.ts` (the training block, §18m), `src/logic/gymSnack.ts` (exercise snacks, §18ab), `src/logic/gymBody.ts` (recovery, §18q), `src/logic/gymStats.ts` (aggregations), `src/logic/gymVideo.ts` (the YouTube link rules, §18n), `src/logic/wakeLock.ts`, `src/screens/GymScreen.tsx` + `src/components/gym/*`, scripts `scripts/gym-equipment.mjs`, `scripts/gym-seed.mjs`, `scripts/gym-demos.mjs`, `scripts/gym-videos.mjs`.

**Six tabs, six URLs** (§1c): `/gym/train` (§18c) · `/gym/snack` (§18ab) · `/gym/blocks` (§18m) · `/gym/body` (§18q) · `/gym/stats` (§18j) · `/gym/gear` (§18k, and 🧠 You — the brief and the session switches, §18g).
...
**Where the load starts (`startLb`).** The weight suggestion is built from your own history, so a loaded exercise you have never logged had *no* weight to suggest — and a carry prescribed with no weight reads as "no weight": nothing shown on the card, nothing typed, nothing remembered, and the next session just as blind. So a loaded exercise can carry a **starting load in pounds**, used only until the first set is logged; after that your history decides and the number is never read again. It is snapped to a real rung of the dumbbell like any other suggestion, meaningless on a fixed load (§18x, the kettlebell is the kettlebell) and an audit error on an unloaded move. Set it in Gym → Gear → tap the exercise → **Starting load**. Today: **Farmer's Carry** and **Dumbbell Farmer's Hold** at 32 lb per hand.

**The setup countdown says the load out loud.** For a clocked *and* loaded set — a carry, a weighted hold — the 15s "get set up" card (§18e) spells out the whole instruction instead of the generic line: *32 lb in each hand, held for 45s. Pick it up now: the clock starts at zero.* Once that set is live your hands are full and the phone is on the floor, so everything you need to read has to be read before it starts.

**One movement, two dumbbells (`loadPerSide`).** A hip-thrust harness takes a dumbbell on *each* end; a farmer's carry is two handles. The number the app stores is then the **per-dumbbell** one, because that is the number you can actually set on the dial and the only one that lands on the rack's real notches (§18d) — and every screen that says a load says both halves: *32 lb each side (64 total)*, the stepper reads `weight per side`, and the brief spells it out before the first set. Flipping the flag on an exercise with history is a **migration, not a checkbox**: `node scripts/gym-load-per-side.mjs <exerciseId>` sets the flag and **halves every load already recorded** for it — finished sessions, the session in progress, and `lastWeight` / `suggestedWeight` / `bestWeight` in the per-exercise memory — so the past reads on the same scale as the future and the next suggestion doesn't come back at double. (`--off` reverses both halves.) Carrying the flag today: **Bench Hip Thrust**, migrated 2026-09-14 — before that date its loads were the single dumbbell laid across the hips, which was the whole load.
...
**The second side is asked to match the first, not the plan.** Prescribed 40 s, held it for 70? The second clock's target is **70**, and the line under it reads *match the first side — 1:10*. That is the point of a per-side hold: the two sides are supposed to be equal, so the number to beat is the one you just set, and a bar that only ever asks for 40 is teaching the strong side nothing. **And it counts down to it.** The first side is a count-up, because nobody knows how long a hold has left in it; the second side's number is already decided, so the digits run *down* to it and the bell rings at zero — the banked zero (§18c-1e), so the two sides are genuinely equal when it goes. Hold past it and the clock turns around and counts the overtime up (`+0:07`), which still gets logged: matching is the ask, not the ceiling.
...
Pressing **↔️ OTHER SIDE** used to start the second clock instantly, which forced a choice between two lies: press it when the side actually dropped and the roll-over is counted as hold time on side two, or press it once you are already set up and side one is credited with seconds it did not hold. So the button now means what it says — *that side is done* — and the runner hands back **5 seconds** (`SIDE_SEC`) on the same countdown the between-sets setup uses (§18c-1), shorter because rolling onto the other forearm is not walking to the rack. The second clock starts at zero when it ends, or the second you tap **▶️ GO NOW**. One beep, at the end: five seconds is already inside the warning window, and two beeps in five seconds is noise.

#### 18c-1d. A hold with no number on it (`maxHold`)
...
The split is by **load, not by gear** — the weights half is `kind === 'weight'` **plus anything flagged `loaded`**, and everything else is bodyweight. A pull-up on a bar is bodyweight; it is equipment you hang from, not weight you add. A farmer's carry is weights work even though the app measures it with a clock. If the filter leaves fewer than three usable exercises — asking for weights-only before a single item of gear has been catalogued — it **falls back to the full pool**, because a real session beats an empty one. The chosen mode is stored on the session and shown on the preview.
...
- **Weight** — read off the **reps**, not off the dumbbell you picked (2026-09-02). *"Loaded less than asked"* is not the same statement as *"that was too heavy"*: choose 48.5 over 52 and complete every set and 48.5 **was** the right weight, so it is kept — dropping below a weight you just finished is how a suggestion oscillates instead of settling. Three outcomes, and the sets say which:
  - **Didn't finish the exercise** → nothing is learned about the load. A session ends early for reasons that have nothing to do with the weight.
  - **Did the work** → that weight is right → it becomes the suggestion. And it is *light* — next suggestion goes up **one notch** — if you either loaded more than asked **or reached the top of the rep range on every working set**.
  - **Couldn't do the work** → too heavy → down **one notch** from what you lifted. Only when you fell **well** short, though: below **70 %** of the ask on a set. A rep or two down is a day, not a verdict, and the same weight comes back next time. (70 % is the same "that ask was wrong" line the rep and hold ladders use.)

  **Warm-up sets are not evidence.** A ramp's opening sets (§18e) are *prescribed* light and long, so they neither trigger the bump nor block it — only the sets at the working weight are read. **A loaded hold is excluded from the bump entirely**: it progresses on the clock, and a movement on two ladders at once has neither of them honest.

  The rep-range half of that rule is **the one place the app asks for something you have not already done**, and it is allowed to because a wrong guess is reversible in a single session. First time on a loaded exercise there is no suggestion; whatever you type becomes the baseline. This reads `loaded`, not `kind`, so a clocked carry learns its weight exactly like a press does — **and learns its seconds too**: a loaded hold progresses on both axes at once, because both are things you actually did.
...
- **How many reps you can do** — the third ladder, and the one the other two leave out: a **counted bodyweight** movement (pull-ups, push-ups, dips) has no weight to add and no clock to beat, so the reps themselves are the progression. `repPlan` stores a set count and a **total**; the shape is derived rather than written down — spare reps land on the **earliest** sets, so 14 over three sets is **5 · 5 · 4**. Hit the number on **every set** and the total goes up by **one rep** next session (5 5 4 → 5 5 5 → 6 5 5). Miss on any of them and the same ask comes back, unchanged, for as many sessions as it takes. Fall below **70 %** of the ask on a set you *did* do and it drops to what you can really manage — but a set you never got to says nothing at all, so stopping a session early never costs you six weeks of ladder. That is **phase 1, volume**, and it ends with every set at the top of the block's range: `3 × 4–8` pull-ups walk from **4 4 4 to 8 8 8**, thirteen sessions at one rep a time. Then **phase 2, density**: the same work spreads over **more sets at fewer reps each** — 3 × 8 becomes **5 × 5**, five sets being the ceiling — with rest held to a strict **120–180 s**, and the one-rep climb starts again from there. **It stops at `sets × (repHigh − 2)`** — 5 × 6 for pull-ups, reached about nineteen sessions in. Forty pull-ups in a session is not strength any more, so the ladder freezes there and the card says the honest next step out loud: **add load** (a vest, a belt, a dumbbell between the feet) **or move to a harder variation**. The Blocks tab marks the slot `ready for load`. Nothing **loaded** gets a rep ladder (a movement with two ladders has neither of them honest) and neither does a ⚡ quality slot. A **40-minute** session's extra set (§18m) is logged like any other but never inflates the ask: what was earned is reps **per set**, and how many sets there are is the block's to say.
...
- **The bands are a colour, not a number** (2026-09-03). Four loop bands hang in the basement — **yellow 20 lb · red 35 lb · black 65 lb · purple 85 lb** — and nothing is printed on any of them. The number on the packet is the vendor's *maximum* pull, and all four are 3 mm rather than the usual 4.5, so every one of them runs lighter than its label: "35 lb" is a fiction and "the red one" is the truth. So any exercise whose gear includes the bands (`eq-bands` — read off the equipment list, never a second field that could drift) gets a **four-rung ladder of colours** in place of the dumbbell's notches. The runner shows **four chips you tap** instead of a −/+ stepper, each with its colour and its pounds underneath for scale; the card reads **"Black band (65 lb)"**; the ask reads **"asked for the red one"**; a logged set's pill says **black**, not 65. Everything *underneath* is unchanged — the stored number is still the pounds, so §18d's up/down-a-notch learning, the records, the grade and the charts all read exactly what they read before, and "one notch up" simply means the next colour. Two consequences: **a band never ramps** (§18e — the ladder is four wide and its two lower rungs sit at 55 %, so a ramp there is a colour change and two identical rungs; prehab and anti-rotation work is not the cold heavy lift a ramp exists to protect, and the way to make a band harder mid-session is to stand further from the anchor), and **the bands have a ladder in kg too**, unlike the dumbbell — four of them is four of them in any unit.
...
**Inside a block session the ramp is a set, not an exercise** (2026-09-02). The free planner's answer above — make the first two *moves* light — cannot work in a block, where the first move of the day is the main lift and lightening it would lighten the session. So the sets climb instead: the first loaded movement for a **body part that has not worked yet in this session** ramps to its working weight rather than opening on it. Each set before the top steps **one notch down the rack** (§18d — the dumbbell has holes, not a dial) and **one rep up**, so `50 lb, 3 × 8–12` is asked for as **42 × 10 · 45.5 × 9 · 48.5 × 8**, and nothing in the ramp ever drops below **55 %** of the working weight.

Three rules keep it from eating the programme:

- **Once per body part.** A part warmed by an earlier movement is not warmed again — in S2 the bench ramps (chest, cold) and so does the row (back, cold), while the pull-ups that follow it do not. It is the exercise's **primary** part that counts.
- **Three sets or more.** A two-set accessory keeps both of its working sets; ramping there would cost half the work it exists to do.
- **Loaded only.** No weight, nothing to ramp — bodyweight work climbs on reps instead (§18d), and a ⚡ quality slot is never ramped.

`plan.weights` carries the per-set load and `plan.weight` stays the **working** weight, so everything that reads one number — the grade, the records, the up/down-a-notch learning — keeps reading the number the session was really about. The runner arms each set with its own prescribed load rather than carrying the last one forward (that rule exists so a bump you make yourself sticks; on a ramp it would flatten the climb), and **walking away before the top set teaches nothing about the load** — an interrupted ramp leaves the working weight exactly where it was, instead of being read as "that was too heavy". A clocked carry ramps on the dumbbells only: 30 s and 31 s are the same hold.
...
**Only off-programme sessions feed it** (2026-09-02). A block session's pull-ups are done under the block's own prescription and its own rep ladder (§18d), so they neither read this state nor advance it — otherwise a free session months later would start from a rung that was never earned under these rules. Inside a block the ladder flags are stripped from the session entirely, so a 🏁 max-test banner can never appear over an ask for `5 · 5 · 4`.
...
**Volume** is reps × weight, and reps alone when there is no weight. A **loaded** hold or carry (§18c-1) is seconds × weight with the seconds first converted to rep-equivalents at the grade's own exchange rate — 6 s to the rep, a cardio minute worth 8 — otherwise a single 45 s carry at 50 lb would outweigh a whole upper-body session on the bars. An **unloaded** hold still counts a second as a unit, exactly as it always has.
...
**And it shows you the bill before you sign it.** Tapping a length or a mood on the card does not change anything yet — it opens a modal that says, slot by slot, what that button does to *today's* session: what comes off the end, what gets an extra set, what the estimate moves to. The modal **never describes the policy**; it builds the session both ways and diffs the two (`diffBlockPlans`), so it cannot drift out of sync with `fitToLength` and a change to the rules shows up here for free. When the two builds come out identical it says so and says why — S3 at 40 minutes is the same session as 30 because both leading slots are ⚡ quality-capped, and **mood never changes a block session at all**: a block decides the work, and the mood is filed with the session as a diary entry, not fed to a planner. Saying that out loud in the one place you would expect it to matter is the point — a control that silently does nothing is worse than no control.
...
**There is no Plan tab any more.** It was the AI trainer's page — a brief for a model to read, and a readiness meter counting down to switching that model off — and the model is gone (§18a). What was still live on it moved to where it belongs: the block you are on, and the warning listing any slot whose exercise has left the catalog, are on the **Blocks** tab; the brief, the four hard rules, the ratings summary and the two session switches are on **Gear → 🧠 You** (§18g). The URL it left behind is now `/gym/snack` (§18ab).
...
**The stepper goes back to the plan afterwards.** The band set asks for about double the reps and the stepper is pre-filled with them; the moment it is logged, the reps and the weight re-arm to what set 1 was actually prescribed. Nothing else does it — no set was logged, so the usual re-arm never fires — and a stepper still holding `20` against the working weight is how twenty reps of the warm-up get logged as the first heavy set.
...
The idea is Huberman's *exercise snacks*: a short, hard, self-contained bout of **one thing**, taken between other things, rather than a workout you had to clear an hour for. Lunch is forty minutes and a training session is not going to fit in it; ten minutes of abs is, and ten minutes done four times a week beats an hour nobody scheduled.

**It is not a second engine, and there must never be one.** A snack builds an ordinary `GymSession` in the ordinary `gym.active` slot and runs through the ordinary runner — so it gets the animation, the ▶ YouTube link (§18n), the rest clock, the 15 s setup, the records (§18p), the grade (§18c-3), the Berries and the streak for free, and the watch drives it without knowing what a snack is. Only the **picking** is different, and the picking is `src/logic/gymSnack.ts`.

**Three routines**, each one theme done three or four ways:
...
Adding a fourth is **one entry in `SNACKS`** — the tab, the recommendation, the planner and the report all read that list.

**The rules the routines live by.**

1. **Bodyweight only**, for now. So progression is **reps** (§18d): `repPlanFor` climbs the ask one rep at a time inside the written range and then spreads it over more sets, the identical ladder a block slot uses. A hold climbs in **seconds** the same way. Ten push-up snacks in a row walk `8 8 8` to `11 11 11` on their own.
2. **The list is fixed**, like a block's. Drop a slot for today, reorder it, or 🔄 swap it for something that does the same job — never "here is a different exercise". A push-up snack that hands you a lateral raise is not a push-up snack, so `isFixedSession()` covers both.
3. **The lower-back rule is not a preference.** A block slot bypasses `avoidBackLoad` by design (§18g); a snack routine ships in code and still filters `backRisk`, because a routine nobody edited is not a decision anybody made.
4. **Fifteen minutes is the ceiling.** Rest is capped at **45 s** whatever your history says — a snack is meant to be dense — and if the routine grows past the cap the **tail** comes off (`fitToSnack`), never the movement the snack is named for. Never below two movements.

**It feeds the Body map, and the Body map feeds it back.** A finished snack is a logged session, so its effort splits across the muscles (§18t) exactly like any other work and tonight's session already knows about it. Running the other way, the tab ranks the three routines on the **same `partRecovery` rows the Body tab draws** — weighted by where each routine's work actually goes, so a fried core does not bury the push-up snack — plus how long since you last did that one, capped at ten days so novelty can't outrank a rested muscle forever.

**⭐ Recommended is a label, not a gate.** The top-ranked routine wears it and every card says *why* in one sentence, naming the part that is actually holding it up rather than the part it mostly trains — those are different questions, and "your core is 4 % back, but it is a small part of this one" is the useful answer on a push-up day. Every routine stays tappable, in any order, whatever the map says. The app's job is to have an opinion and show its working, not to lock a door at 12:40 on a Tuesday.

**It never moves the training block.** No `blockId`, so the rotation cursor does not advance and S*n* is still waiting for you. It does pay Berries, does extend the streak, and does count towards your totals — it is real training.
