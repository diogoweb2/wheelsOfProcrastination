...
- **Weight** — you loaded **more** than asked → too light → next suggestion goes up **one notch**; **less** → too heavy → down one notch; the same → hold. First time on a loaded exercise there is no suggestion; whatever you type becomes the baseline. This reads `loaded`, not `kind`, so a clocked carry learns its weight exactly like a press does — **and learns its seconds too**: a loaded hold progresses on both axes at once, because both are things you actually did.
