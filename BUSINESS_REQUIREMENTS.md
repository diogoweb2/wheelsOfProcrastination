...
- **🎲 The Davy Back Dice — the comeback rule.** When **one more knockout would finish you**, a free roll appears, once a match. **No face is bad**: the worst still hands you ⚡, and the best brings a fallen crewmate back. (It unlocks on the opponent's knockout count, not on cards remaining — with a 4-card deck and 3 knockouts to win, "down to your last card" is a position that never exists, because the match ends on the knockout that gets you there.)
...
- **The bench is the swap control** — tapping one of your own bench cards sends it to the front. On your turn every affordable bench card is outlined in gold, pulses, and carries a thin `SEND ⚡⚡` strip along its bottom edge; unaffordable ones show the cost instead, so it's always clear *why* a card can't come in rather than it just sitting there inert.
