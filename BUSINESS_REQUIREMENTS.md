...
- **A ticked must-do stays on the checklist until midnight**, struck through with its green ✓ and the berries it paid — the day's list is a record of what was asked, so the rows on screen always add up to the "3/5" count above them. This holds even when the tick changes the task underneath: a one-shot must-do archives itself and a rest-days one starts resting immediately, and neither may make the row disappear or turn up in the "do one of the other N today" picker the same day.
