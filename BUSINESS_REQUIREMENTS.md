...
- **Weight** — you loaded **more** than asked → too light → next suggestion goes up **one notch**; **two notches or more below** the ask → too heavy → down one notch from what you lifted; anything in between → the ask comes back as exactly what you lifted. That middle case matters: one notch under the ask is not a failure, it is the notch you settled on, and dropping *below* a weight you just completed is how a suggestion oscillates instead of settling. **And the rep range now buys the notch by itself** (2026-09-02): finish the **last** set — the working set, the one a ramp climbs to — at the **top of the range**, at the weight you were asked for, having done every set, and that is read exactly like loading more than you were asked. Only the top set counts, because a ramp's opening set is *prescribed* at the top of the range on a lighter dumbbell and reading that as proof would ratchet the weight up on a warm-up. This is **the one place the app asks for something you have not already done**, and it is allowed to because it is reversible in a single session. First time on a loaded exercise there is no suggestion; whatever you type becomes the baseline. This reads `loaded`, not `kind`, so a clocked carry learns its weight exactly like a press does — **and learns its seconds too**: a loaded hold progresses on both axes at once, because both are things you actually did.
